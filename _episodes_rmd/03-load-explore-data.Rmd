...
```{r view_example_boxplots_raw, echo=FALSE}
par(las=2)
...
dataset.islet.rnaseq$raw %>%
  ...
  pivot_longer(cols = starts_with("ENS"), 
               ...
               values_to = "counts") %>% 
  group_by(geneID) %>% 
  summarize(gene_average = mean(counts), gene_stdev = sd(counts)) %>% 
  ungroup() %>%
  ...
  ggtitle("Mean - Standard deviation relationship in raw counts")
```
...
```{r visualize_mean_sd_log10, echo=FALSE}
dataset.islet.rnaseq$raw %>%
  as.data.frame() %>% 
  pivot_longer(cols = starts_with("ENS"), 
               names_to = "geneID", 
